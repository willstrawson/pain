...
vps_picture = zeros(numel(unique_subject_ids),9);
vps_video = zeros(numel(unique_subject_ids),37);
...
for i = 1:numel(unique_subject_ids)
    ...
    for cond = 1:36
        ...
        try
            ...
            vps = apply_VPS([subjectID, '_results_video_mni_',suffix]);
            co = str2num(cond) + 1; 
            nps_video(i,co) = nps{1,1};
            vps_video(i,co) = vps{1,1};
...
vps_results = [vps_picture, vps_video(:,2:end)];

save('results.mat' ,'nps_results', 'vps_results');
cd('/its/home/ws231/Downloads/repos/pain/data/source')
writematrix(nps_results,'nps_results_dataset3.csv')
writematrix(vps_results,'vps_results_dataset3.csv')
